        ...
        for m in members:
            if hasattr(m[1], "clidesc"):
                ...
                self.commands[cmd][subcommand] = {
                    ...
                    "optional": len(inspect.getargspec(fn)[3])
                    if inspect.getargspec(fn)[3] else 0,
                    "desc": fn.clidesc,
                    "fn": fn,
                    "order": fn.cliorder
                }
...
############################## cmd input parsing ###############################
    ...
    def print_usage(self):
        ...
        for cmd, subcommands in self.commands.items():
            for subcmd, subcmdDetails in subcommands.items():
                ...
                args = ("%s " % " ".join(
                    ["<%s>" % c
                     for c in subcmdDetails["args"][0:len(subcmdDetails[
                         "args"]) - subcmdDetails["optional"]]]))
                args += ("%s " % " ".join(
                    ["[%s]" % c
                     for c in subcmdDetails["args"][len(subcmdDetails[
                         "args"]) - subcmdDetails["optional"]:]]))
                out += args.ljust(30)
    ...
    def execCmd(self, cmdInput):
        ...
        if len(subcmdData["args"]) < len(args) or len(subcmdData[
                "args"]) - subcmdData["optional"] > len(args):
            return self.print_usage()
    ...
    def startInputThread(self):
        ...
        while (True):
            ...
            cmd = self._queuedCmds.pop(0) if len(self._queuedCmds) else input(
                self.getPrompt()).strip()
